...
let index=0,player,opened=false,flash=0,last=0,completed=false,death=null;
...
function load(i){clearDeath();Sound.recover(true);index=i;player={...rooms[i].spawn};opened=false;completed=false;keys.clear();flash=0;const room=rooms[i];
...
window.addEventListener('keydown',e=>{if(e.target.closest('button,a,input,select,textarea,summary'))return;const key=e.key.toLowerCase();if(['w','a','s','d','arrowleft','arrowright',' ','e'].includes(key)){e.preventDefault();keys.add(key);if(key===' '&&!e.repeat)shoot();if(key==='e'&&!e.repeat)interact();}});
...
function clearDeath(){death=null;canvas.style.transform='';canvas.style.opacity='';const dialog=document.querySelector('#death-dialog');if(dialog.open)dialog.close();}
...
function respawn(){if(!death)return;clearDeath();player={...rooms[index].spawn};keys.clear();lab.dead=false;lab.destroyed=false;lab.lock(false);lab.update();Sound.recover();document.querySelector('#loaded-status').textContent=lab.loaded?`● Cargada: ${lab.loaded.name} · ${lab.loaded.mass} kg`:'Recámara vacía. Elige una bala y cárgala.';message.textContent=opened?'Has reaparecido. El puente sigue activo.':'Has reaparecido. Ajusta tu próximo intento.';document.querySelector('#feedback').textContent='Nuevo intento: conservas el registro. Revisa la energía antes de disparar.';canvas.focus();}
...
function tick(time){const dt=Math.min((time-last)/1000,.04);last=time;flash=Math.max(0,flash-dt);
 ...
 if(!supported(player.x,player.y))die('fall');
